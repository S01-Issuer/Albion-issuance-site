		...
		{#if holdings.length > 0}
			<ContentSection background="white" padding="standard">
				...
				<div class="grid grid-cols-1 gap-4 lg:gap-6">
						...
						{#if group.fieldName.toLowerCase().includes('wressle')}
							<details class="mt-1 mb-2 border border-yellow-400 bg-yellow-50">
								<summary class="flex items-center gap-2 px-4 py-2.5 cursor-pointer text-sm font-medium text-yellow-900 select-none">
									<svg class="w-4 h-4 text-yellow-600 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
									...
									A distribution is overdue
								</summary>
								<div class="px-4 pb-3 text-sm text-yellow-900">
									The royalty originator has received funds to distribute to tokenholders but is currently resolving paperwork issues with their on-ramp service.
								</div>
							</details>
						{/if}
					{/each}
