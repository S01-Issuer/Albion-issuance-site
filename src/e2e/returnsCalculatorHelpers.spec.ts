	...
	const baseToken: TokenMetadata = {
		...
		asset: {
			...
			plannedProduction: {
				...
				projections: [
					{ month: '2025-01', production: 1000, revenue: 80000 },
					{ month: '2025-02', production: 950, revenue: 76000 },
					{ month: '2025-03', production: 900, revenue: 72000 },
					{ month: '2025-04', production: 850, revenue: 68000 },
					{ month: '2025-05', production: 800, revenue: 64000 },
					{ month: '2025-06', production: 750, revenue: 60000 },
					{ month: '2025-07', production: 700, revenue: 56000 },
					{ month: '2025-08', production: 650, revenue: 52000 },
					{ month: '2025-09', production: 600, revenue: 48000 },
					{ month: '2025-10', production: 550, revenue: 44000 },
					{ month: '2025-11', production: 500, revenue: 40000 },
					{ month: '2025-12', production: 450, revenue: 36000 },
				],
			},
...
describe('returnsCalculatorHelpers', () => {
	describe('calculateMonthlyTokenCashflows', () => {
		it('returns empty array for token with no projections', () => {
			const token = makeToken();
			if (token.asset) {
				token.asset.plannedProduction = { oilPriceAssumption: 80, projections: [] };
			}
		...
		it('scales future cashflows by numberOfTokens (accounting for adjusted supply)', () => {
			...
			const result1 = calculateMonthlyTokenCashflows(token, 80, mintedSupply, 1);
			const result2 = calculateMonthlyTokenCashflows(token, 80, mintedSupply, 2);
			...
			for (let i = 1; i < Math.min(result1.length, result2.length); i++) {
				expect(result2[i].cashflow / result1[i].cashflow).toBeCloseTo(expectedRatio, 2);
			}
		});
		...
		it('applies benchmark premium and transport costs', () => {
			...
			const resultAdjusted = calculateMonthlyTokenCashflows(tokenWithPricing, 80, 1000, 1);

			// Adjusted price should affect cashflows
			if (resultBase.length > 1 && resultAdjusted.length > 1) {
				expect(resultAdjusted[1].cashflow).toBeGreaterThan(resultBase[1].cashflow);
			}
		});
	});
	...
	describe('getLifetimeCashflows', () => {
		...
		it('includes all months from firstPaymentDate without pro-rating', () => {
			...
			const monthlyCashflows = calculateMonthlyTokenCashflows(token, 80, 1000, 1);

			// Lifetime should have more or equal cashflows (includes all months)
			expect(lifetimeCashflows.length).toBeGreaterThanOrEqual(monthlyCashflows.length);
		});
	});
	...
	describe('calculateNPV', () => {
		...
		it('NPV decreases as discount rate increases', () => {
			...
			const npv3 = calculateNPV(cashflows, 0.10);
	...
	describe('Integration tests', () => {
		it('APR calculation works for different token amounts', () => {
			...
			const cashflows1 = calculateMonthlyTokenCashflows(token, 80, 1000, 1).map(c => c.cashflow);
			const cashflows5 = calculateMonthlyTokenCashflows(token, 80, 1000, 5).map(c => c.cashflow);
			const cashflows10 = calculateMonthlyTokenCashflows(token, 80, 1000, 10).map(c => c.cashflow);
			...
			const calculateAPR = (cashflows: number[], investment: number) => {
				...
				if (countPeriods > 0 && totalReturns > 0 && investment > 0) {
					return (Math.pow(totalReturns / investment, 12 / countPeriods) - 1) * 100;
				}
				return -99;
			};
